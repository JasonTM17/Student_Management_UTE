...
test('redact paths rewrites windows absolute paths in warnings too', () => {
  const payload = {
    repo: { root: 'C:\\Users\\Admin\\Repo' },
    current: { root: 'C:\\Users\\Admin\\Repo', worktree: { path: 'C:\\Users\\Admin\\Repo' } },
    worktrees: [{ path: 'C:\\Users\\Admin\\Repo' }],
    plans: {
      unfinished: [{ id: 'C:\\Users\\Admin\\Repo\\plans\\launch\\plan.md', sources: [] }],
      completedRecent: [],
    },
    roadmaps: [],
    nextSteps: [{ action: 'Check C:\\Users\\Admin\\Repo\\plans\\launch\\plan.md', rationale: 'Fix C:\\Temp\\scratch\\notes.txt' }],
    warnings: ['could not read C:\\Users\\Admin\\Repo\\plans\\launch\\plan.md: Access denied'],
  };

  const { redactPaths } = require('./watzup-scan.cjs');
  redactPaths(payload);

  assert.equal(JSON.stringify(payload).includes('C:\\Users\\Admin\\Repo'), false);
  assert.ok(payload.warnings[0].includes('<path:'));
  assert.ok(payload.nextSteps[0].action.includes('<path:'));
});
