...
Add `STITCH_API_KEY=sk_...` to `~/.agentkit/.env` (or the project's
adapter-local `.env`, such as `.codex/.env`).
...
```bash
# In ~/.agentkit/.env
STITCH_PROJECT_ID="my-project"    # Default project (auto-creates "agentkit-default" if unset)
STITCH_QUOTA_LIMIT="200"          # Override daily limit
```

### 4. MCP Server (optional)

Register the server with the active runtime. For a project-local Codex
adapter, keep AgentKit metadata in `.codex/.mcp.json` and use Codex's normal
MCP registration flow for the actual runtime connection:
...
- Local tracking via `~/.agentkit/.stitch-quota.json` (or
  `$AGENTKIT_RUNTIME_HOME/.stitch-quota.json` when configured)
