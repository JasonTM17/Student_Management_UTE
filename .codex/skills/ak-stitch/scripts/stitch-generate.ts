 ...
 *   5. "agentkit-default"      last resort fallback
 */
...
const QUOTA_DIR = process.env.AGENTKIT_RUNTIME_HOME
  ? path.resolve(process.env.AGENTKIT_RUNTIME_HOME)
...
if (directProjectId) {
  ...
  projectId = detected || "agentkit-default";
  resolvedProjectName = projectId;
  isNameBasedProject = true;
  if (detected) {
...
async function main() {
  try {
    ...
    if (remaining < creditsNeeded) {
      ...
      console.error("[i] Use ak:ui-ux-pro-max as fallback, or wait until midnight UTC.");
      process.exit(2);
    }
    ...
    if (err.code === "RATE_LIMITED") {
      ...
      q.count = q.limit;
      saveQuota(q);
      console.error("[X] Daily quota exceeded (local tracker synced). Try tomorrow or use ak:ui-ux-pro-max.");
