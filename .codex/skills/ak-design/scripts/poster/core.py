...
COMMON_SCRIPTS = SKILL_ROOT.parent / "ak-common" / "scripts"
...
def load_env() -> None:
    """Load .env from the project adapter and AgentKit user runtime."""
    loaded: dict[str, str] = {}
    candidates = environment_paths(SKILL_ROOT)
...
def get_api_key() -> str:
    ...
    if not key:
        raise RuntimeError(
            "GEMINI_API_KEY not set. Add it to project .env, the active adapter .env, or ~/.agentkit/.env."
        )
    return key
