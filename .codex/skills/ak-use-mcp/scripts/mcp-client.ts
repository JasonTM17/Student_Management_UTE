...
export class MCPClientManager {
  ...
  async loadConfig(configPath?: string): Promise<MCPConfig> {
    const fullPath = resolve(process.cwd(), configPath || defaultConfigPath());
...
/** Resolve a project-local MCP JSON file without assuming one runtime vendor. */
export function defaultConfigPath(): string {
  const explicit = process.env.MCP_CONFIG_PATH?.trim();
  if (explicit) return explicit;

  const adapters = ['.codex', '.claude', '.cursor', '.agents'];
  let current = process.cwd();
  for (;;) {
    for (const adapter of adapters) {
      const candidate = resolve(current, adapter, '.mcp.json');
