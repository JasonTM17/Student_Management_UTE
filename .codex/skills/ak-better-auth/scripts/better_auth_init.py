...
.env loading order: process.env > skill/.env > adapter/skills/.env > adapter/.env > project/.env > ~/.agentkit/.env
"""
...
from runtime_paths import environment_paths, project_root_for_skill
...
class BetterAuthInit:
    ...
    def _find_project_root() -> Path:
        """
        Find project root from the current project or this installed skill.
        ...
        """
        return project_root_for_skill(Path(__file__).parent.parent)

    def _load_env_files(self) -> Dict[str, str]:
        """
        Load environment variables from .env files in order.

        Loading order: process.env > skill/.env > adapter/skills/.env >
        adapter/.env > project/.env > ~/.agentkit/.env.
        ...
        env_paths = environment_paths(skill_dir, project_root=self.project_root)
