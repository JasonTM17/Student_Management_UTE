...
function cleanupOrphanedShadowedSkills() {
  const shadowedDir = path.join(process.cwd(), '.codex', 'skills', '.shadowed');
...
/**
 * Codex must not infer a team from another runtime's global state. Native Codex
 * team/cooperation context is supplied by its own runtime and project agents.
...
async function main() {
  ...
  try {
    ...
    if (envFile) {
      ...
      // AgentKit task-list identity for multi-session/subagent coordination.
      // Task list ID = plan directory name (shared across all sessions working on same plan)
      if (taskListId) {
        writeEnv(envFile, 'CK_TASK_LIST_ID', taskListId);
      }
    ...
    // This Codex adapter deliberately avoids reading global state from other runtimes.
    ...
    if (hasCleanup) {
      ...
      if (shadowedCleanup.kept.length > 0) {
        ...
        console.log(`    Review .codex/skills/.shadowed/ and merge changes manually.`);
      }
    }
    ...
    if (teamInfo) {
      ...
      console.log(`    Team context is supplied by the active Codex runtime.`);
    ...
    if (source === 'compact') {
      ...
      console.log(`exhaust device memory. Follow the repository AGENTS.md process-management rules.`);
    ...
    const guidelines = getCodingLevelGuidelines(codingLevel, path.join(baseDir, '.codex'));
    if (guidelines) {
      console.log(`\n${guidelines}`);
    }
