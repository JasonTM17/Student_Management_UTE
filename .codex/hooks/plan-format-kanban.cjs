 ...
 * Reads the Codex hook payload, checks file_path/content and apply_patch targets
 * for bad link text patterns.
...
function safeProjectPath(value, root) {
  if (typeof value !== 'string' || !value.trim()) return null;
  const raw = value.trim().replace(/^['"]|['"]$/g, '');
  const candidate = canonicalPath(path.isAbsolute(raw) ? raw : path.resolve(root, raw));
...
function applyPatchTargets(command, root) {
  if (typeof command !== 'string' || !command) return [];
  const targets = [];
  const marker = /\*\*\*\s+(?:Update|Add|Delete) File:\s*([^\r\n]+)/g;
  for (const match of command.matchAll(marker)) {
    const target = safeProjectPath(match[1], root);
...
process.stdin.on('end', () => {
  ...
  try {
    ...
    const explicitPath = safeProjectPath(
      data.tool_input?.file_path || data.tool_input?.path,
      root,
    );
    const patchTargets = toolName.toLowerCase() === 'apply_patch'
      ? applyPatchTargets(data.tool_input?.command, root)
      : [];
    const filePath = explicitPath || patchTargets.find((candidate) => path.basename(candidate).toLowerCase() === 'plan.md') || '';
    ...
    if (toolName === 'Edit' || toolName === 'Write' || toolName.toLowerCase() === 'apply_patch') {
      const toolOutput = toolName.toLowerCase() === 'apply_patch'
        ? data.tool_input?.command || ''
        : data.tool_input?.new_string || data.tool_input?.content || '';
