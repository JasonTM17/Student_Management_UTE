 ...
 */
function resolveRulesPath(filename, configDirName = ".codex") {
	...
	for (const directory of [configDirName]) {
		if (fs.existsSync(path.join(os.homedir(), directory, "rules", filename))) return `~/${directory}/rules/${filename}`;
	}
	...
	for (const directory of [configDirName]) {
		if (fs.existsSync(path.join(os.homedir(), directory, "workflows", filename))) return `~/${directory}/workflows/${filename}`;
	}
 ...
 * @returns {string|null} Resolved path or null
 */
function resolveScriptPath(filename, configDirName = ".codex") {
	...
	for (const directory of [configDirName]) {
		if (fs.existsSync(path.join(os.homedir(), directory, "scripts", filename))) return `~/${directory}/scripts/${filename}`;
	}
	return null;
}
 ...
 */
function resolveSkillsVenv(configDirName = ".codex") {
	...
	for (const directory of [configDirName]) {
		const globalVenv = path.join(os.homedir(), directory, "skills", ".venv", venvBin, pythonExe);
		if (fs.existsSync(globalVenv)) {
			return isWindows ? `~/${directory}/skills/.venv/Scripts/python.exe` : `~/${directory}/skills/.venv/bin/python3`;
		}
	}
	return null;
}
...
function buildRulesSection({ devRulesPath, skillsVenv, plansPath, docsPath }) {
	...
	if (skillsVenv) {
		lines.push(`- Python skill scripts: Use \`${skillsVenv}\``);
	}
 ...
 * }}
 */
function buildReminderContext({ sessionContext, config, staticEnv, configDirName = ".codex", baseDir } = {}) {
