...
def _nearest_project_config_candidates() -> list[tuple[Path, str]]:
    """Return the closest project adapter configs, preferring Codex."""
    current = Path.cwd().resolve()
    for directory in [current, *current.parents]:
        adapters = (".codex", ".claude", ".cursor", ".agents")
        if not any((directory / adapter).is_dir() for adapter in adapters):
            continue
        ...
        for adapter in adapters:
            ...
            if not adapter_dir.is_dir():
                continue
            candidates.append((adapter_dir / ".mcp.json", "json"))
            if adapter == ".codex":
                candidates.append((adapter_dir / "config.toml", "toml"))
        return candidates
    return []
...
def _load_mcp_config_summary() -> dict:
    ...
    candidates.extend(_nearest_project_config_candidates())
    candidates.extend([
        (Path.home() / ".codex" / ".mcp.json", "json"),
...
def detect_bridge_state(
    ...
    if cdp_payload:
        ...
        state["remediation"].append(
            "Add chrome-devtools-mcp to the active project adapter config (for Codex, .codex/config.toml), then restart the agent session."
        )
        return state
