...
Expose the graph as an MCP server for the active AgentKit runtime to query directly:
...
For a Codex project adapter, add to `.codex/.mcp.json` and register the server
through Codex's normal MCP setup flow:
```json
{
  "mcpServers": {
    "graphify": {
