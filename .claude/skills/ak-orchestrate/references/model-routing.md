...
- `task`: scout, architecture, adversarial, implement, review, audit, security,
  test, docs, or mechanical;
...
| `scout` | C1 | R0 |
| `architecture` | C3 | R0 |
| `adversarial` | C3 | R0; Wukong remains report-only |
...
- prefer the live `wukong` agent for an `adversarial` job only when the
  expected output is falsification evidence and a gate, never implementation or
  sign-off;
