...
def render(
    ...
    try:
        ...
        print(f"Run: cd {Path(__file__).resolve().parent} && uv sync && uv run playwright install chromium", file=sys.stderr)
        sys.exit(1)
    ...
    with sync_playwright() as p:
        try:
            ...
            if "Executable doesn't exist" in str(e) or "browserType.launch" in str(e):
                ...
                print(f"Run: cd {Path(__file__).resolve().parent} && uv run playwright install chromium", file=sys.stderr)
                sys.exit(1)
            raise
