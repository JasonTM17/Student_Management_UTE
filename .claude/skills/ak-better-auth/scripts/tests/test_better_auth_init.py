...
class TestBetterAuthInit:
    ...
    def test_find_project_root_falls_back_to_installed_skill(self, tmp_path, monkeypatch):
        """An arbitrary cwd falls back to the project containing the skill."""
        no_package_dir = tmp_path / "no-package"
        no_package_dir.mkdir()
        monkeypatch.chdir(no_package_dir)

        init = BetterAuthInit()
        assert init.project_root != no_package_dir
        assert init.project_root.exists()
...
"""
    ...
    def test_load_env_files(self, auth_init, mock_project_root):
        ...
        # Create .env files
        adapter_env = mock_project_root / ".codex" / ".env"
        adapter_env.parent.mkdir(parents=True, exist_ok=True)
        adapter_env.write_text("BASE_VAR=base\nOVERRIDE=adapter")
