...
| **Direct Scripts** | A server exists only in the active project adapter MCP JSON (for Codex, `.codex/.mcp.json`), or the task needs a deterministic tool name and argument shape. | Explicit and reproducible; requires local dependencies. |
...
the active project adapter MCP JSON that are not registered with the active runtime, or for
...
- Direct scripts read the active project adapter MCP JSON; do not silently copy credentials into another runtime's global config.
