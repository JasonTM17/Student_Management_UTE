...
```bash
git clone https://github.com/7onez/cti-expert.git ${CLAUDE_PLUGIN_ROOT}/skills/ak-cti-expert
bash ${CLAUDE_PLUGIN_ROOT}/skills/ak-cti-expert/scripts/install.sh
```
...
```powershell
git clone https://github.com/7onez/cti-expert.git "$env:USERPROFILE\.agentkit\skills\cti-expert"
pip3 install -r "$env:USERPROFILE\.agentkit\skills\cti-expert\scripts\requirements.txt"
```
...
3. **Extract to your skills folder** &mdash; Unzip the downloaded file, then move the extracted folder to your skills directory and rename it to `cti-expert`:
   ...
   | **Windows** | Open **File Explorer** &rarr; Type `%USERPROFILE%\.agentkit\skills\` in the address bar &rarr; Press **Enter** &rarr; Move the folder here |

   > **Note:** If the `skills` folder does not exist, create it inside the `.agentkit` folder first.
...
3. **Giải nén vào thư mục skills** &mdash; Giải nén file đã tải, di chuyển thư mục vào thư mục skills và đổi tên thành `cti-expert`:
   ...
   | **Windows** | Mở **File Explorer** &rarr; Nhập `%USERPROFILE%\.agentkit\skills\` vào thanh địa chỉ &rarr; Nhấn **Enter** |
...
3. **解压到 skills 文件夹** &mdash; 解压文件，将文件夹重命名为 `cti-expert` 并移动到：
   ...
   | **Windows** | `%USERPROFILE%\.agentkit\skills\` （文件资源管理器地址栏） |
