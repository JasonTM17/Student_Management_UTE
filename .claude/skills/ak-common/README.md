...
3. **Active project adapter `.env` file**
   ```bash
   echo 'GEMINI_API_KEY=your-api-key' > .codex/.env
   ```

   On Claude Code or Cursor, the same lookup uses that project's active
   adapter directory.  User-level fallback is `~/.agentkit/.env`, never a
   provider-specific credential directory.
