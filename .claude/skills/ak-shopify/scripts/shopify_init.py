...
class EnvLoader:
    ...
    def get_env_paths(skill_dir: Path) -> List[Path]:
        ...
        Priority: process.env > skill/.env > adapter/skills/.env >
        ...
        """
        return [path for path in environment_paths(skill_dir) if path.exists()]
    ...
    def load_config(skill_dir: Path) -> EnvConfig:
        ...
        # environment_paths() is already ordered lowest to highest file
        # priority, so later project-local/skill-local values override the
        # lower-level runtime defaults before process.env takes final priority.
        for env_path in EnvLoader.get_env_paths(skill_dir):
