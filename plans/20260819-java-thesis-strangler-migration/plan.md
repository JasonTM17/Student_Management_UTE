...
- Current committed head: `cd8696f` (`fix: add mobile cards for admin identity views`).
...
- Six Admin Users/Courses/Classrooms/Departments/Semesters/Lecturers views now have bounded stacked mobile-card implementations with desktop-table fallbacks; the latest frontend smoke is 24/24, typecheck and lint pass, and the remaining dense admin tables are still open.
