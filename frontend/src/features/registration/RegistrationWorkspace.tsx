...
export default function RegistrationWorkspace() {
  ...
  const validationCloseRef = useRef<HTMLButtonElement>(null);
  const { confirm, confirmationDialog } = useConfirmationDialog();
  const closeValidation = useCallback(() => setValidationOpen(false), []);
  const validationDialogRef = useDialogFocusTrap<HTMLElement>({
    open: validationOpen,
    onClose: closeValidation,
    initialFocusRef: validationCloseRef,
    restoreFocusRef: validationTriggerRef,
  });
  ...
  const commitSelection = async () => {
    ...
    if (!confirmed) return;
    setMutating(true); closeValidation(); setNotice('');
    try {
