...
function stateClass(state: string) {
  if (state === 'PUBLISHED') return 'bg-emerald-500/12 text-emerald-700 dark:text-emerald-300';
  if (state === 'PENDING_REVIEW') return 'bg-amber-500/15 text-amber-700 dark:text-amber-300';
  if (state === 'ARCHIVED') return 'bg-secondary text-muted-foreground';
...
export default function AdminAssistantKnowledgePage() {
  ...
  return (
    ...
    >
      ...
      {notice ? <div role="status" className="mb-5 flex items-center gap-2 border border-emerald-500/30 bg-emerald-500/5 px-4 py-3 text-sm text-emerald-700 dark:text-emerald-300"><Check className="h-4 w-4" aria-hidden="true" />{notice}</div> : null}

      <div className="space-y-6">
        <div className="grid gap-3 sm:grid-cols-3">
          <AdminMetricCard label={vi ? 'Tổng nguồn' : 'Total sources'} value={documents.length} icon={<FileText className="h-5 w-5" />} toneClassName="bg-blue-500/12 text-blue-600 dark:text-blue-400" compact />
          <AdminMetricCard label={vi ? 'Đã xuất bản' : 'Published'} value={publishedCount} icon={<ShieldCheck className="h-5 w-5" />} toneClassName="bg-emerald-500/12 text-emerald-600 dark:text-emerald-400" compact />
          <AdminMetricCard label={vi ? 'Chờ duyệt' : 'Pending review'} value={pendingCount} icon={<UploadCloud className="h-5 w-5" />} toneClassName="bg-amber-500/12 text-amber-600 dark:text-amber-400" compact />
        </div>
        ...
        <AdminTableCard title={vi ? 'Nguồn curated' : 'Curated sources'} description={vi ? 'Nguồn được kiểm duyệt trước khi trợ lý sử dụng. Archive là soft archive để giữ audit và revision.' : 'Sources are reviewed before assistant retrieval. Archive is a soft archive that preserves audit and revisions.'}>
          ...
          {isLoading && documents.length === 0 ? <LoadingState label={vi ? 'Đang tải nguồn...' : 'Loading sources...'} /> : documents.length === 0 ? <p className="py-12 text-center text-sm text-muted-foreground">{vi ? 'Chưa có nguồn phù hợp.' : 'No matching sources.'}</p> : <AdminTableScroll><table className="w-full min-w-[760px] text-left text-sm"><thead className="border-b border-border/70 text-xs uppercase tracking-[0.08em] text-muted-foreground"><tr><th className="px-3 py-3 font-semibold">{vi ? 'Nguồn' : 'Source'}</th><th className="px-3 py-3 font-semibold">{vi ? 'Trạng thái' : 'State'}</th><th className="px-3 py-3 font-semibold">{vi ? 'Phiên bản' : 'Version'}</th><th className="px-3 py-3 text-right font-semibold">{vi ? 'Thao tác' : 'Actions'}</th></tr></thead><tbody>{documents.map((document) => <tr key={document.documentId} className="border-b border-border/60 align-top last:border-0"><td className="max-w-[420px] px-3 py-4"><div className="font-semibold text-foreground">{document.title}</div><div className="mt-1 truncate text-xs text-muted-foreground">{document.slug} · {document.locale}</div><p className="mt-2 line-clamp-2 text-sm leading-5 text-muted-foreground">{document.content}</p></td><td className="px-3 py-4"><span className={`inline-flex rounded-full px-2.5 py-1 text-xs font-semibold ${stateClass(document.state)}`}>{document.state}</span></td><td className="px-3 py-4 tabular-nums text-muted-foreground">v{document.version}</td><td className="px-3 py-4"><div className="flex flex-wrap justify-end gap-2"><Button type="button" size="sm" variant="outline" onClick={() => openEdit(document)} disabled={isSaving}><Edit3 className="mr-1.5 h-4 w-4" aria-hidden="true" />{vi ? 'Sửa' : 'Edit'}</Button>{document.state === 'DRAFT' ? <Button type="button" size="sm" variant="outline" onClick={() => void transition(document, 'submit')} disabled={isSaving}><Send className="mr-1.5 h-4 w-4" aria-hidden="true" />{vi ? 'Gửi duyệt' : 'Submit'}</Button> : null}{document.state === 'PENDING_REVIEW' ? <Button type="button" size="sm" onClick={() => void transition(document, 'publish')} disabled={isSaving}><ShieldCheck className="mr-1.5 h-4 w-4" aria-hidden="true" />{vi ? 'Xuất bản' : 'Publish'}</Button> : null}{document.state !== 'ARCHIVED' ? <Button type="button" size="sm" variant="ghost" onClick={() => { setError(''); setArchiveTarget(document); }} disabled={isSaving}><Archive className="mr-1.5 h-4 w-4" aria-hidden="true" />{vi ? 'Lưu trữ' : 'Archive'}</Button> : null}</div></td></tr>)}</tbody></table></AdminTableScroll>}
        </AdminTableCard>
      </div>
