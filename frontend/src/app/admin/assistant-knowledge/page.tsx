    ...
    >
      ...
      <div className="space-y-6">
        ...
        <AdminTableCard title={vi ? 'Nội dung đã kiểm duyệt' : 'Reviewed guidance'} description={vi ? 'Nội dung được kiểm duyệt trước khi trợ lý sử dụng. Nội dung đã lưu trữ vẫn giữ lịch sử thay đổi.' : 'Guidance is reviewed before the assistant uses it. Archived items keep their change history.'}>
          ...
          {isLoading && documents.length === 0 ? <LoadingState label={vi ? 'Đang tải nội dung...' : 'Loading guidance...'} /> : documents.length === 0 ? <EmptyState title={vi ? 'Chưa có nội dung phù hợp.' : 'No matching guidance.'} description={vi ? 'Nội dung mới sẽ xuất hiện tại đây sau khi được tạo hoặc khi bộ lọc thay đổi.' : 'New guidance will appear here once created or when the filters change.'} /> : <AdminTableScroll><table className="w-full min-w-[760px] text-left text-sm"><thead className="bg-secondary text-xs uppercase tracking-[0.08em] text-muted-foreground"><tr><th className="px-3 py-3 font-semibold">{vi ? 'Nội dung' : 'Guidance'}</th><th className="px-3 py-3 font-semibold">{vi ? 'Lĩnh vực' : 'Domain'}</th><th className="px-3 py-3 font-semibold">{vi ? 'Trạng thái' : 'Status'}</th><th className="px-3 py-3 font-semibold">{vi ? 'Lần cập nhật' : 'Revision'}</th><th className="px-3 py-3 text-right font-semibold">{vi ? 'Thao tác' : 'Actions'}</th></tr></thead><tbody>{documents.map((document) => <tr key={document.documentId} className="border-b border-border/60 align-top last:border-0"><td className="max-w-[420px] px-3 py-4"><div className="font-semibold text-foreground">{document.title}</div><div className="mt-1 truncate text-xs text-muted-foreground">{document.source}</div></td><td className="px-3 py-4"><span className="inline-flex rounded-md bg-secondary px-2.5 py-1 text-xs font-semibold text-secondary-foreground">{domainLabel(document.domain, vi)}</span></td><td className="px-3 py-4"><span className={`inline-flex rounded-md px-2.5 py-1 text-xs font-semibold ${stateClass(document.state)}`}>{stateLabel(document.state, vi)}</span></td><td className="px-3 py-4 tabular-nums text-muted-foreground">v{document.version}</td><td className="px-3 py-4"><div className="flex flex-wrap justify-end gap-2"><Button type="button" size="sm" variant="outline" onClick={() => void openEdit(document)} disabled={isSaving}><Edit3 className="mr-1.5 h-4 w-4" aria-hidden="true" />{vi ? 'Sửa' : 'Edit'}</Button>{document.state === 'DRAFT' ? <Button type="button" size="sm" variant="outline" onClick={() => void transition(document, 'submit')} disabled={isSaving}><Send className="mr-1.5 h-4 w-4" aria-hidden="true" />{vi ? 'Gửi duyệt' : 'Send for review'}</Button> : null}{document.state === 'PENDING_REVIEW' ? <Button type="button" size="sm" onClick={() => void transition(document, 'publish')} disabled={isSaving}><ShieldCheck className="mr-1.5 h-4 w-4" aria-hidden="true" />{vi ? 'Xuất bản' : 'Release'}</Button> : null}{document.state !== 'ARCHIVED' ? <Button type="button" size="sm" variant="ghost" onClick={() => setArchiveTarget(document)} disabled={isSaving}><Archive className="mr-1.5 h-4 w-4" aria-hidden="true" />{vi ? 'Lưu trữ' : 'Archive'}</Button> : null}</div></td></tr>)}</tbody></table></AdminTableScroll>}
        </AdminTableCard>
      </div>

      <Modal isOpen={formOpen} onClose={() => setFormOpen(false)} title={editing ? (vi ? 'Sửa nội dung đã kiểm duyệt' : 'Edit reviewed guidance') : (vi ? 'Tạo nội dung mới' : 'Add guidance')} className="max-w-4xl">
        <div className="space-y-4"><div className="grid gap-4 sm:grid-cols-3"><label className="space-y-2 text-sm font-medium text-foreground"><span>{vi ? 'Mã nội dung' : 'Guidance code'}</span><Input value={form.slug} onChange={(event) => setForm((current) => ({ ...current, slug: event.target.value }))} placeholder="registration-window" /></label><label className="space-y-2 text-sm font-medium text-foreground"><span>{vi ? 'Lĩnh vực' : 'Domain'}</span><select className="h-11 w-full rounded-md border border-input bg-background px-3 text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring" value={form.domain ?? 'GENERAL_FAQ'} onChange={(event) => setForm((current) => ({ ...current, domain: event.target.value as AssistantKnowledgeDomain }))}>{domains.filter((option) => option.value).map((option) => <option key={option.value} value={option.value}>{vi ? option.vi : option.en}</option>)}</select></label><label className="space-y-2 text-sm font-medium text-foreground"><span>{vi ? 'Ngôn ngữ' : 'Language'}</span><select className="h-11 w-full rounded-md border border-input bg-background px-3 text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring" value={form.locale} onChange={(event) => setForm((current) => ({ ...current, locale: event.target.value as AssistantKnowledgeRequest['locale'] }))}><option value="both">{vi ? 'Cả hai' : 'Both'}</option><option value="vi">Tiếng Việt</option><option value="en">English</option></select></label></div><label className="space-y-2 text-sm font-medium text-foreground"><span>{vi ? 'Tiêu đề' : 'Title'}</span><Input value={form.title} onChange={(event) => setForm((current) => ({ ...current, title: event.target.value }))} /></label><label className="space-y-2 text-sm font-medium text-foreground"><span>{vi ? 'Nguồn tham chiếu' : 'Reference'}</span><Input value={form.source} onChange={(event) => setForm((current) => ({ ...current, source: event.target.value }))} /></label><div className="space-y-2 text-sm font-medium text-foreground"><span>{vi ? 'Nội dung quy chế / tri thức' : 'Guidance content'}</span><RichTextEditor value={form.content} onChange={(newContent) => setForm((current) => ({ ...current, content: newContent }))} locale={vi ? 'vi' : 'en'} minHeight="240px" hint={vi ? 'Chỉ nhập thông tin học vụ công khai. Không thêm hồ sơ hoặc dữ liệu cá nhân.' : 'Only public campus information. Do not add profiles or personal data.'} placeholder={vi ? 'Biên soạn nội dung quy chế hoặc hướng dẫn học vụ... (Hỗ trợ tiêu đề, bảng biểu, danh sách, callout)' : 'Draft academic guidance content... (Supports headings, tables, lists, callouts)'} /></div><div className="flex justify-end gap-2 border-t border-border/70 pt-4"><Button type="button" variant="outline" onClick={() => setFormOpen(false)} disabled={isSaving}>{vi ? 'Hủy' : 'Cancel'}</Button><Button type="button" onClick={() => void save()} disabled={isSaving}>{isSaving ? (vi ? 'Đang lưu...' : 'Saving...') : (vi ? 'Lưu bản nháp' : 'Save draft')}</Button></div></div>
      </Modal>
