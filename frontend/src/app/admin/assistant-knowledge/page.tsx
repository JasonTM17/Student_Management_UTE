...
export default function AdminAssistantKnowledgePage() {
  ...
  if (authLoading || isLoggingOut) {
    return <AdminFrame title={vi ? 'Hướng dẫn luận văn' : 'Thesis guidance'} description={vi ? 'Quản trị nội dung học thuật công khai.' : 'Manage public academic guidance for the assistant.'}><LoadingState /></AdminFrame>;
  }

  if (!user || !canAccess) {
    return <AdminFrame title={vi ? 'Hướng dẫn luận văn' : 'Thesis guidance'} description={vi ? 'Quản trị nội dung học thuật công khai.' : 'Manage the academic guidance used by the assistant.'}><ForbiddenState title={vi ? 'Không có quyền truy cập' : 'Access restricted'} description={vi ? 'Chỉ quản trị viên có thể quản lý nội dung đã công bố.' : 'Only administrators can manage published guidance.'} action={<LinkButton href="/dashboard" variant="outline">{vi ? 'Về trang tổng quan' : 'Return to overview'}</LinkButton>} /></AdminFrame>;
  }

  return (
    <AdminFrame
      title={vi ? 'Hướng dẫn luận văn' : 'Thesis guidance'}
      description={vi ? 'Quản trị nội dung học thuật công khai theo quy trình bản nháp, duyệt và xuất bản.' : 'Manage public academic guidance through draft, review, and release states.'}
    ...
    >
      ...
      <div className="space-y-6">
        ...
        <Card variant="muted">
          <CardHeader className="pb-3"><CardTitle className="flex items-center gap-2"><BookOpenCheck className="h-5 w-5 text-primary" aria-hidden="true" />{vi ? 'Phạm vi nội dung công khai' : 'Public guidance coverage'}</CardTitle><CardDescription>{vi ? 'Chỉ hiển thị số lượng nội dung học thuật công khai. Không bao gồm hồ sơ, điểm, điểm danh, lịch cá nhân hoặc danh sách lớp.' : 'A quick count of public academic guidance. Personal records, grades, attendance, schedules, and class lists are not included.'}</CardDescription></CardHeader>
          <CardContent>
      ...
      <Modal isOpen={formOpen} onClose={() => setFormOpen(false)} title={editing ? (vi ? 'Sửa nội dung đã kiểm duyệt' : 'Edit reviewed guidance') : (vi ? 'Tạo nội dung mới' : 'Add guidance')} className="max-w-3xl">
        <div className="space-y-4"><div className="grid gap-4 sm:grid-cols-2"><label className="space-y-2 text-sm font-medium text-foreground"><span>{vi ? 'Mã nội dung' : 'Guidance code'}</span><Input value={form.slug} onChange={(event) => setForm((current) => ({ ...current, slug: event.target.value }))} placeholder="thesis-registration" /></label><label className="space-y-2 text-sm font-medium text-foreground"><span>{vi ? 'Ngôn ngữ' : 'Language'}</span><select className="h-11 w-full rounded-md border border-input bg-background px-3 text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring" value={form.locale} onChange={(event) => setForm((current) => ({ ...current, locale: event.target.value as AssistantKnowledgeRequest['locale'] }))}><option value="both">{vi ? 'Cả hai' : 'Both'}</option><option value="vi">Tiếng Việt</option><option value="en">English</option></select></label></div><label className="space-y-2 text-sm font-medium text-foreground"><span>{vi ? 'Tiêu đề' : 'Title'}</span><Input value={form.title} onChange={(event) => setForm((current) => ({ ...current, title: event.target.value }))} /></label><label className="space-y-2 text-sm font-medium text-foreground"><span>{vi ? 'Nguồn tham chiếu' : 'Reference'}</span><Input value={form.source} onChange={(event) => setForm((current) => ({ ...current, source: event.target.value }))} /></label><label className="space-y-2 text-sm font-medium text-foreground"><span>{vi ? 'Nội dung' : 'Guidance content'}</span><Textarea value={form.content} onChange={(event) => setForm((current) => ({ ...current, content: event.target.value }))} className="min-h-[220px]" hint={vi ? 'Chỉ nhập thông tin học thuật công khai. Không thêm hồ sơ hoặc dữ liệu cá nhân.' : 'Only public academic information. Do not add profiles or personal data.'} /></label><div className="flex justify-end gap-2 border-t border-border/70 pt-4"><Button type="button" variant="outline" onClick={() => setFormOpen(false)} disabled={isSaving}>{vi ? 'Hủy' : 'Cancel'}</Button><Button type="button" onClick={() => void save()} disabled={isSaving}>{isSaving ? (vi ? 'Đang lưu...' : 'Saving...') : (vi ? 'Lưu bản nháp' : 'Save draft')}</Button></div></div>
      </Modal>
