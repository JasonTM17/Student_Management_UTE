...
type VerificationState = 'idle' | 'checking' | 'success' | ChallengeFailureState;
type ResetState = 'ready' | 'saving' | 'success' | ChallengeFailureState;
...
function challengeFailureState(error: unknown): ChallengeFailureState {
  const code = errorCode(error);
  if (code === 'AUTH_CHALLENGE_EXPIRED') return 'expired';
  if (code === 'AUTH_CHALLENGE_ATTEMPTS_EXCEEDED') return 'attemptsExceeded';
  if (code === 'AUTH_CHALLENGE_INVALID') return 'invalid';
  return 'unavailable';
}

function useAlertFocus(active: boolean) {
...
const lifecycleLinkClassName =
  'inline-flex min-h-11 items-center rounded-sm font-medium text-primary underline-offset-4 transition-transform duration-150 hover:underline active:translate-y-px focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 motion-reduce:transform-none';
...
export function RegisterPage() {
  ...
  const errorRef = useAlertFocus(Boolean(error));
    ...
    >
      <div className="space-y-6">
        <div className="space-y-2">
          <SectionEyebrow className="lg:hidden">{copy.sectionEyebrow}</SectionEyebrow>
        ...
        {success ? (
          ...
          <form onSubmit={submit} className="space-y-4" noValidate aria-busy={pending} aria-describedby={error ? 'register-error' : undefined}>
            {error ? <div id="register-error" ref={errorRef} role="alert" tabIndex={-1} className="rounded-md border border-destructive/30 bg-destructive/5 px-4 py-3 text-sm text-destructive focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-destructive">{error}</div> : null}
            ...
            <Button type="submit" className="w-full" disabled={pending}>
              {pending ? copy.submitting : copy.submit}<ArrowRight className="ml-2 h-4 w-4" aria-hidden="true" />
            </Button>
          </form>
        )}
        <LifecycleFooter>{copy.alreadyHaveAccount}{' '}<LocalizedLink href="/login" className={lifecycleLinkClassName}>{copy.signIn}</LocalizedLink></LifecycleFooter>
      </div>
    </AuthShell>
  );
}
...
export function VerifyEmailPage() {
  ...
  const verificationError = state === 'invalid' || state === 'expired' || state === 'attemptsExceeded' || state === 'unavailable';
  const verificationErrorRef = useAlertFocus(verificationError);
  ...
  useEffect(() => {
    ...
    void authApi.confirmEmail(rawToken.trim())
      ...
      .catch((requestError) => setState(challengeFailureState(requestError)));
  ...
  const confirm = async (candidate = token) => {
    ...
    try {
      ...
      setState(challengeFailureState(requestError));
    }
  };
  ...
  return (
    <AuthShell eyebrow={copy.eyebrow} title={copy.title} description={copy.description} features={copy.featureTitles.map((label, index) => ({ label, description: copy.featureDescriptions[index] }))}>
      <div className="space-y-6">
        <div className="space-y-2"><SectionEyebrow className="lg:hidden">{copy.sectionEyebrow}</SectionEyebrow><h2 className="text-2xl font-semibold leading-8 text-foreground">{copy.heading}</h2><p className="text-sm leading-6 text-muted-foreground">{verificationMessage}</p></div>
        ...
        {state !== 'success' ? <form className="space-y-4" aria-busy={state === 'checking'} onSubmit={(event) => { event.preventDefault(); void confirm(); }}>
          {verificationError ? <div ref={verificationErrorRef} role="alert" tabIndex={-1} className="rounded-md border border-destructive/30 bg-destructive/5 px-4 py-3 text-sm text-destructive focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-destructive">{verificationMessage}</div> : null}
        ...
        {state !== 'success' ? <LifecycleFooter><LocalizedLink href="/login" className={lifecycleLinkClassName}>{copy.backToLogin}</LocalizedLink></LifecycleFooter> : null}
      </div>
    </AuthShell>
  );
}
...
export function ForgotPasswordPage() {
  ...
  return <AuthShell eyebrow={copy.eyebrow} title={copy.title} description={copy.description} features={copy.featureTitles.map((label, index) => ({ label, description: copy.featureDescriptions[index] }))}>
    <div className="space-y-6"><div className="space-y-2"><SectionEyebrow className="lg:hidden">{copy.sectionEyebrow}</SectionEyebrow><h2 className="text-2xl font-semibold leading-8 text-foreground">{copy.heading}</h2><p className="text-sm leading-6 text-muted-foreground">{sent ? copy.afterSend : copy.beforeSend}</p></div>
      {error ? <div ref={errorRef} role="alert" tabIndex={-1} className="rounded-md border border-destructive/30 bg-destructive/5 px-4 py-3 text-sm text-destructive focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-destructive">{error}</div> : null}
      {sent ? <div role="status" className="rounded-md border border-emerald-500/30 bg-emerald-500/5 px-4 py-4 text-sm"><p>{copy.sentBanner.replace('{email}', email.trim())}</p><p className="mt-2 text-muted-foreground">{copy.sentDescription}</p></div> : <form onSubmit={submit} className="space-y-4" aria-busy={pending}><div className="space-y-2 text-sm font-medium"><label htmlFor="forgot-email">{copy.emailLabel}</label><Input id="forgot-email" type="email" icon={<Mail className="h-4 w-4" />} value={email} onChange={(event) => setEmail(event.target.value)} placeholder={copy.emailPlaceholder} autoComplete="email" hint={copy.emailHint} required /></div><Button type="submit" className="w-full" disabled={pending}>{pending ? copy.sendingResetInstructions : copy.sendResetLink}</Button></form>}
      <LifecycleFooter><LocalizedLink href="/login" className={lifecycleLinkClassName}>{messages.login.heading}</LocalizedLink></LifecycleFooter></div>
  </AuthShell>;
}
...
export function ResetPasswordPage() {
  ...
  const resetError = state === 'invalid' || state === 'expired' || state === 'attemptsExceeded' || state === 'unavailable';
  const resetErrorRef = useAlertFocus(resetError || Boolean(formError));
  ...
  const submit = async (event: React.FormEvent) => {
    event.preventDefault();
    setFormError('');
    if (!token.trim()) { setState('invalid'); return; }
  ...
  const errorText = useMemo(() => {
    ...
    if (state === 'unavailable') return copy.errors.fallback;
    return copy.invalidDescription;
  }, [copy.attemptsExceededDescription, copy.errors.fallback, copy.expiredDescription, copy.invalidDescription, state]);
  ...
  return <AuthShell eyebrow={copy.eyebrow} title={copy.title} description={copy.description} features={copy.featureTitles.map((label, index) => ({ label, description: copy.featureDescriptions[index] }))}>
    <div className="space-y-6"><div className="space-y-2"><SectionEyebrow className="lg:hidden">{copy.sectionEyebrow}</SectionEyebrow><h2 className="text-2xl font-semibold leading-8 text-foreground">{copy.heading}</h2><p className="text-sm leading-6 text-muted-foreground">{state === 'success' ? copy.successToast : resetError ? errorText : copy.subheading}</p></div>
      ...
      {state !== 'success' ? <form onSubmit={submit} className="space-y-4" aria-busy={state === 'saving'}>{resetError ? <div ref={resetErrorRef} role="alert" tabIndex={-1} className="rounded-md border border-destructive/30 bg-destructive/5 px-4 py-4 text-sm text-destructive focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-destructive"><strong>{errorTitle}</strong><p className="mt-1">{errorText}</p></div> : null}{formError && !resetError ? <div ref={resetErrorRef} role="alert" tabIndex={-1} className="rounded-md border border-destructive/30 bg-destructive/5 px-4 py-3 text-sm text-destructive focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-destructive">{formError}</div> : null}<div className="space-y-2 text-sm font-medium"><label htmlFor="reset-token">{copy.tokenLabel}</label><Input id="reset-token" type="password" value={token} onChange={(event) => { setToken(event.target.value); setState('ready'); setFormError(''); }} placeholder={copy.tokenPlaceholder} autoComplete="off" required /></div><div className="space-y-2 text-sm font-medium"><label htmlFor="reset-password">{copy.newPassword}</label><Input id="reset-password" type="password" icon={<Lock className="h-4 w-4" />} value={password} onChange={(event) => setPassword(event.target.value)} placeholder={copy.newPasswordPlaceholder} autoComplete="new-password" minLength={8} hint={copy.minimumHint} required /></div><div className="space-y-2 text-sm font-medium"><label htmlFor="reset-confirm-password">{copy.confirmPassword}</label><Input id="reset-confirm-password" type="password" icon={<Lock className="h-4 w-4" />} value={confirmPassword} onChange={(event) => setConfirmPassword(event.target.value)} placeholder={copy.confirmPasswordPlaceholder} autoComplete="new-password" minLength={8} required /></div><Button type="submit" className="w-full" disabled={state === 'saving'}>{state === 'saving' ? copy.savePassword : copy.resetPassword}</Button></form> : null}
      ...
      {state !== 'success' ? <LifecycleFooter><LocalizedLink href="/forgot-password" className={lifecycleLinkClassName}>{messages.forgotPassword.heading}</LocalizedLink></LifecycleFooter> : null}
    </div>
  </AuthShell>;
}
