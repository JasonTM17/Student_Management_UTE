...
Flyway là schema authority duy nhất. V13-V20 là migration forward-only, có
...
Identity/session của ứng dụng nằm trong `campuscore_auth`; V20 đưa các object
CampusCore ra khỏi schema `auth` do Supabase quản lý. Supabase mới dùng B20
schema-only sau identity/backup/drift gate; đây không phải production cutover.
