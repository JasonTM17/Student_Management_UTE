-- Fail closed before Flyway validates or executes the legacy CampusCore
-- stream.  A hosted target must use the exact, schema-only
-- db/supabase-baseline location, which does not load this callback.
DO $$
DECLARE
    auth_owner text;
    auth_exists boolean;
