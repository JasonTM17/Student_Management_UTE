...
public class ThesisAssistantRepository {
    ...
    public UUID ensureConversation(String ownerId, String requestedId, String locale, int retentionDays) {
        if (requestedId != null && !requestedId.isBlank()) {
            ...
            Integer count = jdbc.queryForObject("SELECT COUNT(*) FROM assistant.chat_conversation WHERE id=:id AND owner_id=:owner AND state IN ('ACTIVE','PENDING') AND expires_at > CURRENT_TIMESTAMP",
        ...
        jdbc.update("INSERT INTO assistant.chat_conversation (id, owner_id, locale, state, expires_at) VALUES (:id,:owner,:locale,'PENDING',:expires)",
                p().addValue("id", id).addValue("owner", ownerId).addValue("locale", normalizeLocale(locale))
                        // PostgreSQL's JDBC driver cannot infer a SQL type for
                        // java.time.Instant through NamedParameterJdbcTemplate.
                        // Bind an explicit timestamp so Postgres and H2 share
                        // the same retention behavior.
    ...
    public UUID appendMessage(UUID conversationId, String role, String content, String model, boolean degraded, String reasonCode) {
        UUID id = UUID.randomUUID();
        jdbc.update("INSERT INTO assistant.chat_message (id,conversation_id,role,content,model,degraded,reason_code) VALUES (:id,:conversation,:role,:content,:model,:degraded,:reason)",
        ...
        jdbc.update("UPDATE assistant.chat_conversation SET state=CASE WHEN :role='ASSISTANT' THEN 'ACTIVE' ELSE state END,updated_at=CURRENT_TIMESTAMP WHERE id=:id",
    ...
    public void appendCitations(UUID messageId, List<ThesisAssistantDtos.Citation> citations) {
        for (ThesisAssistantDtos.Citation citation : citations) {
            jdbc.update("INSERT INTO assistant.chat_citation (id,message_id,document_id,slug,title,source,locale,excerpt) VALUES (:id,:message,:document,:slug,:title,:source,:locale,:excerpt)",
    ...
    public int purgeExpired() {
        ...
        for (Conversation conversation : expired) {
            ...
            List<UUID> locked = jdbc.query("SELECT id FROM assistant.chat_conversation WHERE id=:id AND owner_id=:owner AND expires_at <= CURRENT_TIMESTAMP AND state <> 'PURGED' FOR UPDATE",
                    p().addValue("id", conversation.id()).addValue("owner", conversation.ownerId()),
                    (rs, row) -> rs.getObject("id", UUID.class));
            if (locked.isEmpty()) continue;
            Integer active = jdbc.queryForObject("SELECT COUNT(*) FROM assistant.chat_turn_ledger WHERE owner_id=:owner AND conversation_id=:id AND state IN ('RESERVED','SNAPSHOT_READY','DISPATCHED')",
        ...
        List<BucketKey> oldBuckets = jdbc.query("SELECT bucket_date,owner_id,scope FROM assistant.usage_bucket WHERE bucket_date < CURRENT_DATE - 90 ORDER BY bucket_date,owner_id,scope LIMIT 100",
                new MapSqlParameterSource(), (rs, row) -> new BucketKey(rs.getObject("bucket_date", LocalDate.class), rs.getString("owner_id"), rs.getString("scope")));
        for (BucketKey bucket : oldBuckets) {
            jdbc.update("DELETE FROM assistant.usage_bucket WHERE bucket_date=:date AND owner_id=:owner AND scope=:scope",
                    p().addValue("date", bucket.date()).addValue("owner", bucket.ownerId()).addValue("scope", bucket.scope()));
        }
        jdbc.update("DELETE FROM assistant.chat_turn_ledger WHERE state='PURGED' AND tombstone_until < CURRENT_TIMESTAMP AND NOT EXISTS (SELECT 1 FROM assistant.chat_conversation c WHERE c.id=assistant.chat_turn_ledger.conversation_id)", new MapSqlParameterSource());
        ...
        jdbc.update("DELETE FROM assistant.provider_dispatch_registry r WHERE r.state <> 'DISPATCHED' AND NOT EXISTS (SELECT 1 FROM assistant.chat_turn_ledger l WHERE l.owner_id=r.owner_id AND l.client_request_id=r.client_request_id AND l.lease_generation=r.lease_generation)", new MapSqlParameterSource());
        return deleted;
    }
    ...
    private void ensureBucket(LocalDate date, String owner, String scope) {
        try {
            ...
            // A concurrent request created the bucket; the caller locks it next.
        }
    }
    ...
    public int deleteConversation(UUID id, String ownerId) {
        ...
        jdbc.update("UPDATE assistant.provider_dispatch_registry r SET state='CANCELLED',cancelled_at=CURRENT_TIMESTAMP WHERE r.owner_id=:owner AND EXISTS (SELECT 1 FROM assistant.chat_turn_ledger l WHERE l.owner_id=r.owner_id AND l.client_request_id=r.client_request_id AND l.conversation_id=:id)",
                p().addValue("id", id).addValue("owner", ownerId));
        return jdbc.update("DELETE FROM assistant.chat_conversation WHERE id=:id AND owner_id=:owner", p().addValue("id", id).addValue("owner", ownerId));
    }
    ...
    public MessagePage messagesPage(UUID conversationId, String ownerId, int requestedLimit, String cursor) {
        ...
        MapSqlParameterSource params = p().addValue("id", conversationId).addValue("owner", ownerId).addValue("limit", limit + 1);
        if (boundary != null) params.addValue("cursorTime", Timestamp.from(boundary.instant())).addValue("cursorId", boundary.id());
        List<Message> newestFirst = jdbc.query("SELECT m.id,m.role,m.content,m.model,m.degraded,m.reason_code,m.created_at,(SELECT f.rating FROM assistant.chat_message_feedback f WHERE f.message_id=m.id AND f.owner_id=:owner) feedback FROM assistant.chat_message m JOIN assistant.chat_conversation c ON c.id=m.conversation_id WHERE c.id=:id AND c.owner_id=:owner AND c.state='ACTIVE' AND c.expires_at>CURRENT_TIMESTAMP" + predicate + " ORDER BY m.created_at DESC,m.id DESC LIMIT :limit",
