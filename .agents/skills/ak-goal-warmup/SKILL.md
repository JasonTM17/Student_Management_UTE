...
For high-risk or assumption-heavy plans, prefer `/ak:wukong --mode
shadow-review` when installed. Pass the locked contract and target identity;
accept findings only, then classify them here. Wukong must not edit the plan,
replace the final reviewer, or silently change the approved outcome.
