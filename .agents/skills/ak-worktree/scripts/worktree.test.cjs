 ...
 * Run: node <skill-root>/scripts/worktree.test.cjs
 */
...
const MONOREPO_DIR = process.env.MONOREPO_DIR || '/path/to/claudekit';
const CURRENT_GIT_ROOT = path.resolve(execSync('git rev-parse --show-toplevel', {
  encoding: 'utf-8',
  cwd: STANDALONE_DIR,
...
const TEST_TMP_ROOT = fs.mkdtempSync(path.join(os.tmpdir(), 'ak-worktree-tests-'));
...
function runNode(scriptArgs, options = {}) {
  const result = spawnSync(process.execPath, [SCRIPT_PATH, ...scriptArgs], {
    encoding: 'utf-8',
    cwd: options.cwd || STANDALONE_DIR,
...
test('create dry-run shows explicit base branch source', () => {
  const result = run(`create test-explicit-base --dry-run --json --base "${CURRENT_BRANCH}"`);
  assert(result.success, 'Should succeed with explicit base');
  const json = assertJSON(result.output);
  assert(json.wouldCreate.baseBranch === CURRENT_BRANCH, 'Should use explicit base branch');
...
test('create --worktree-root overrides default location', () => {
  const customRoot = path.join(TEST_TMP_ROOT, 'custom-worktrees');
...
test('superproject detection in submodule', () => {
  ...
  const submodulePath = path.join(MONOREPO_DIR, 'claudekit-engineer');
...
test('WORKTREE_ROOT env var overrides detection', () => {
  const envRoot = path.join(TEST_TMP_ROOT, 'env-worktrees');
  const result = runNode(['create', 'test-env', '--prefix', 'feat', '--dry-run', '--json'], {
    env: { ...process.env, WORKTREE_ROOT: envRoot }
  });
  assert(result.success, result.stderr || 'Should succeed with WORKTREE_ROOT env');
  const json = assertJSON(result.output);
...
test('invalid WORKTREE_ROOT env var fails safely', () => {
  const invalidRoot = path.join(TEST_TMP_ROOT, 'missing', 'deep', 'worktrees');
  const result = runNode(['info', '--json'], {
    env: { ...process.env, WORKTREE_ROOT: invalidRoot }
  });
  assert(!result.success, 'Should fail with invalid WORKTREE_ROOT');
  const json = assertJSON(result.output);
...
test('create --worktree-root validates path existence', () => {
  ...
  const invalidRoot = path.join(TEST_TMP_ROOT, 'nonexistent', 'deeply', 'nested', 'path');
...
test('non-git directory returns error', () => {
  const nonGitDir = fs.mkdtempSync(path.join(TEST_TMP_ROOT, 'non-git-'));
  const result = run('info --json', { cwd: nonGitDir });
...
test('create handles path with spaces via --worktree-root', () => {
  const pathWithSpaces = path.join(TEST_TMP_ROOT, 'my worktree dir');
...
test('create validates file path as worktree root', () => {
  const fileRoot = path.join(TEST_TMP_ROOT, 'not-a-directory');
  fs.writeFileSync(fileRoot, 'test fixture');
  const result = run(`create test-file --prefix feat --json --worktree-root "${fileRoot}"`);
