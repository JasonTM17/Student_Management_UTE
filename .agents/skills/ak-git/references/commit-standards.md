...
- Treat AgentKit adapter/skill updates as behavioral when they change runtime
  behavior; use `feat`, `fix`, or `perf` as appropriate rather than treating
  every adapter file as documentation.
...
## Special Cases
- AgentKit skill updates: `perf(skill): improve token efficiency`
- New AgentKit skills: `feat(skill): add database-optimizer`
