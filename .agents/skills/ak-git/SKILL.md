...
Execute git workflows via `git-manager` when that runtime agent is available,
but keep the primary controller responsible for explicit path scope and user
authorization for commits, pushes, merges, and PR writes.
...
### Step 1: Inspect + Stage Explicitly
```bash
git status --short
git diff --check
git diff -- <approved-path-1> <approved-path-2>
git add -- <approved-path-1> <approved-path-2>
git diff --cached --stat -- <approved-path-1> <approved-path-2>
```

Never use `git add -A`, `git add .`, a broad glob, or a reset to hide unrelated
work. If unrelated files are already staged, stop and ask the owner before
changing the index.

### Step 2: Security Check
Scan for secrets before commit:
```bash
git diff --cached --binary | rg -q -i "(api[_-]?key|token|password|secret|credential)"
```
If the scan exits zero, STOP and report only that credential-shaped material
was detected; do not print matching values. Suggest environment variables or
an ignore rule, then wait for the owner.

### Step 3: Split Decision

**NOTE:**
- Search for related issues on GitHub and add to body.
- Choose the commit type from the actual intent, including AgentKit adapter
  changes; do not classify every runtime file as documentation.
