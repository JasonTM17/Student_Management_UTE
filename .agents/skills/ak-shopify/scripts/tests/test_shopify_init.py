...
""")
    ...
    def test_get_env_paths(self, tmp_path, monkeypatch):
        """Test getting .env file paths."""
        # Create directory structure
        adapter_dir = tmp_path / ".codex"
        ...
        (adapter_dir / ".env").write_text("ADAPTER=1")
        ...
        assert adapter_dir / ".env" in paths
    ...
    def test_load_config_priority(self, tmp_path, monkeypatch):
        ...
        (adapter_dir / ".env").write_text("SHOPIFY_API_KEY=adapter_key\nSHOP_DOMAIN=adapter.myshopify.com")
    ...
    def test_load_config_file_priority(self, tmp_path, monkeypatch):
        """A skill .env wins over shared adapter and project values."""
        adapter_dir = tmp_path / ".codex"
        skills_dir = adapter_dir / "skills"
        skill_dir = skills_dir / "shopify"
        skill_dir.mkdir(parents=True)
        ...
        with monkeypatch.context() as context:
            context.delenv("SHOPIFY_API_KEY", raising=False)
            config = EnvLoader.load_config(skill_dir)

        assert config.shopify_api_key == "skill_key"
