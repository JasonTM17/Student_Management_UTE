...
# Repository attributes keep this Bash entrypoint on LF across host platforms.

set -euo pipefail

SKILL_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
# Keep optional shared dependencies in an AgentKit-owned user runtime.  The
# path can be overridden for CI or managed installations without referring to
# a Claude-specific home directory.
RUNTIME_HOME="${AGENTKIT_RUNTIME_HOME:-$HOME/.agentkit}"
VENV_ROOT="${AGENTKIT_VENV_ROOT:-$RUNTIME_HOME/skills/.venv}"
...
if [[ ! -f "$VENV_PIP" ]]; then
  echo -e "  ${RED}✘${NC} Venv not found at $VENV_ROOT"
  echo "  Run: python3 -m venv \"$VENV_ROOT\""
  exit 1
fi
...
SHARETRACE_DIR="$RUNTIME_HOME/skills/ak-cti-expert/vendor/sharetrace"
