...
Three options for connecting Google Stitch as an MCP server with AgentKit.
...
### 2. Add project metadata to `.codex/.mcp.json`
...
Restart the active runtime after registering the MCP server. You should see
...
For Codex, use its normal MCP registration flow. `.codex/.mcp.json` is the
project-local AgentKit metadata source; for another runtime, place equivalent
metadata in that runtime's adapter directory instead of copying a Claude-only
configuration file.
