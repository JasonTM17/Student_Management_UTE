...
- Text: .md, .txt
- Documents: .pdf, .docx, .xlsx, .pptx (via multix doc convert)
- Media: .jpg, .jpeg, .png, .webp, .mp4, .mov (via multix gemini analyze)
...
try:
    # JSON mode deliberately preserves Vietnamese/Unicode names.  Make its
    # byte stream deterministic on Windows consoles instead of inheriting a
    # legacy code page such as CP1252.
...
COMMON_SCRIPTS = Path(__file__).resolve().parents[2] / "ak-common" / "scripts"
if str(COMMON_SCRIPTS) not in sys.path:
    sys.path.insert(0, str(COMMON_SCRIPTS))

from runtime_paths import project_root_for_skill


PROJECT_ROOT = project_root_for_skill(Path(__file__).parent.parent)
STYLES_DIR = PROJECT_ROOT / 'assets' / 'writing-styles'
MULTIX_COMMAND = ['npx', '-y', '-p', '@mrgoonie/multix@0.2.0', 'multix']
...
def extract_document_content(file_path: Path, verbose: bool = False) -> str:
    """Extract content from documents using multix doc convert."""
...
def extract_media_content(file_path: Path, verbose: bool = False) -> str:
    """Extract writing style from media using multix gemini analyze."""
    try:
