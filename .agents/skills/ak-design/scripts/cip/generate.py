...
COMMON_SCRIPTS = Path(__file__).resolve().parents[3] / "ak-common" / "scripts"
...
def load_env():
    """Load environment variables from the active project adapter."""
    loaded = {}
    for env_path in environment_paths(Path(__file__).resolve().parents[2]):
        if env_path.exists():
  ...
  to incorporate your ACTUAL logo into the CIP mockups.
    ...
    if args.logo:
        ...
        if action == 'generate':
            ...
            print(f"   python ~/.agentkit/skills/ak-design/scripts/logo/generate.py --brand \"{args.brand}\" --industry \"{args.industry}\"")
